...
*Setup instructions for a Gitian build of VGCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the VGCoin
...
- [Building VGCoin Core](#building-VGCoin-core)
...
Clone the git repositories for VGCoin and Gitian.
...
```bash
sudo cp /home/debian/VGCoin/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the VGCoin repository under 'Fetch and create inputs' to install sources which require
...
To build VGCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the VGCoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/VGCoin/.git/
...
For example:
```bash
URL=https://github.com/laanwj/VGCoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit VGCoin=${COMMIT} --url VGCoin=${URL} ../VGCoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit VGCoin=${COMMIT} --url VGCoin=${URL} ../VGCoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit VGCoin=${COMMIT} --url VGCoin=${URL} ../VGCoin/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-VGCoin:
./bin/gbuild --commit VGCoin=${COMMIT},cpp-eth-VGCoin=${ETHCOMMIT} --url VGCoin=${URL},cpp-eth-VGCoin=${ETHURL} ../VGCoin/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the VGCoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../VGCoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/VGCoin
SIGPATH=/some/root/path/VGCoin-detached-sigs

./bin/gbuild --url VGCoin=${BTCPATH},signature=${SIGPATH} ../VGCoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/VGCoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/VGCoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/VGCoin-osx-build.assert
```
...
[VGCproject/gitian.sigs](https://github.com/VGCproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@VGCoin.org) and he will commit them.
